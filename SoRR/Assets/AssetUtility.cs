...
namespace SoRR
{
    ...
    public static partial class AssetUtility
    {
        ...
        [Pure] public static async Task<AudioClip> CreateAudioClipAsync(byte[] rawData)
        {
            ...
            if (format == AudioType.UNKNOWN) throw new FormatException("The audio file's format could not be detected.");

            string tempFileName = Path.GetTempFileName();
            try
            {
                await File.WriteAllBytesAsync(tempFileName, rawData);
                return await CreateAudioClipAsync(tempFileName, format);
            }
            finally
            {
        ...
        [Pure] public static Task<AudioClip> CreateAudioClipAsync(string filePath, AudioType format)
        {
            ...
            operation.completed += _ => source.SetFromFunc(() => DownloadHandlerAudioClip.GetContent(uwr));
